...
void main() {
  ...
  // vec2 distorted_uvs_0 = vec2(atlas_0.x + uBlend * (disp_0.r * 1.0), atlas_0.y);
  ...
  // vec2 distorted_uvs_1 = vec2(atlas_1.x - (1.0 - uBlend) * (disp_1.r * 1.0), atlas_1.y);

  // vec4 texture_1 = texture(uNextTexture, atlas_1);

  /////////////////////////////////////////////////////////////////////////////////
  vec2 blendUV = mix(atlas_0, atlas_1, uBlend);
  vec4 t0 = texture(uTexture, blendUV);
  vec4 t1 = texture(uNextTexture, blendUV);
  vec4 tt = mix(t0, t1, uBlend);

  // vec4 finalTexture = mix(texture_0, texture_1, uBlend);
  // fragColor = vec4(LinearTosRGB(finalTexture).xyz, 1.);
  fragColor = vec4(tt.xyz, 1.);
  /////////////////////////////////////////////////////////////////////////////////

  // vec2 vis = mix(distorted_uvs_0, distorted_uvs_1, uBlend);
  // vec2 visD = mix(distorted_uvs_0, distorted_uvs_1, uBlend);
  // if(vUv.x + vUv.y > 1.) {
